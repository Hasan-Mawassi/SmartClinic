...
export const get_available_slots = (doctor) =>
  tool(
    async ({ text }) => {
    ...
    },
    {
      ...
      }. Extract a date from the user message and return the available time slots for the doctor on that date 
        in a numbered list.`,
      schema: textSchema,
    }
  );
...
export const book_appointemnt = (userId, doctor) =>
  tool(
    async ({ index }) => {
      if (!available.length) {
      ...
      const result = await bookAppointmentByIndex(
        available,
        index,
        userId,
        doctor.id
